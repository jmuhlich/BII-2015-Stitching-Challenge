...
fp = '\\ITLNAS\bio-data\Stitching\Stitching Challenge BII 2015\';
fp = 'C:\majurski\image-data\BII_Stitching_Challenge\';

% loop across the participants
participants = dir([fp 'Participants']);
...
for p = 1:numel(participants)
  ...
  for l = 1:3
    ...
    tmp = {tmp.name}';
    if numel(tmp) ~= 1
      warning(['cannot find positions csv file for ' participants{p} ' at level ' num2str(l)]);
      continue;
    end
    csv_fp = [csv_fp tmp{1}];
