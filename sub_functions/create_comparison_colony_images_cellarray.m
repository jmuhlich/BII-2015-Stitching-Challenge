...
% get individual colony image by cropping from the full image
for i = 1:nb_cell
  % extract the mask corresponding to the current object label
  A = S(bounding_box(i,1):bounding_box(i,2), bounding_box(i,3):bounding_box(i,4)) == i;
  % crop out subregion of full image
...
% exteact centroinds for all the labeled objects
s = regionprops(S,'Centroid');
c = [s.Centroid];
c = reshape(c,2,[])';
% convert centroid coordinates from pixels to microns
