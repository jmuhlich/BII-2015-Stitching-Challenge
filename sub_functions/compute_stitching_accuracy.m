...
% ref_raw_images:     the individual recentered colonies from the reference image
% ref_seg_images:     the individual segmented  colonies from the reference image
% ref_colony_positions:     the individual reference colony stage locations
% stitched_raw_images:     the colony images from the comparison stitched image
...
function [FN,FP,total_area_error,distance_error, ref_colony_positions, ref_raw_images, ref_seg_images, ref_colony_ind,...
  stitched_colony_positions, stitched_raw_images, stitched_seg_images, stitched_colony_ind, R] = compute_stitching_accuracy(ref_raw_images,...
...
for i = 1:nb_ref_colonies
  ...
  if k ~= 0
    ...
    % if there is not an object pixel in the middle of the image
    discard_stitched_ref_images(i) = 1;
  end
end
% remove the invalid/discarded images
